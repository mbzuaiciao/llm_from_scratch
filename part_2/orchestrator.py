...
if __name__ == '__main__':
    ...
    run("uv run python train.py --data tiny_hi.txt --steps 400 --sample_every 100 --eval_interval 100 --batch_size 32 --block_size 128 --n_layer 2 --n_head 2 --n_embd 128")

    # sample from the best checkpoint
    run(f"uv run python sample.py --ckpt {RUNS}/model_best.pt --tokens 200 --prompt 'Once upon a time '")

    # evaluate final val loss
    run(f"uv run python eval_loss.py --data tiny_hi.txt --ckpt {RUNS}/model_best.pt --iters 50 --block_size 128")